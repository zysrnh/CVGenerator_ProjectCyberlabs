...
${skillsText ? `<div class="section-title-ep">Skills</div><div class="section-content"><p class="skills-text">${skillsText}</p></div>` : ''}
    ...
    const downloadPDF = async () => {
        ...
        // 1. Save ke database
        const safeDOB = formData.date_of_birth && /^\d{4}-\d{2}-\d{2}$/.test(formData.date_of_birth)
            ? formData.date_of_birth
            : null;

        const { photo: _photo, ...dataOnly } = { ...formData };
        const dataToSubmit = { ...dataOnly, date_of_birth: safeDOB };
        try {
            const response = await fetch('/cv-submissions-slokavia', {
                method: 'POST',
                headers: {
            ...
            let result = {};
            try { result = await response.json(); } catch { /* bukan JSON */ }

            if (result.success) {
                showNotification('success', 'CV Saved!', `${formData.full_name} — Data has been stored`);
            } else {
            ...
            showNotification('error', 'Save Failed', `Network error: ${err.message}`);
        }

        // 2. Generate PDF via iframe print (same method as Turkey template)
        try {
            const html = generateCVHTML(formData, photoData, euLogoData);
    ...
    const prevStep = () => {
        ...
        else window.location.href = '/import/slokavia';
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100" style={{ fontFamily: "'Poppins', sans-serif" }}>
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-12 animate-scale-in">
                    ...
                    {currentStep === 1 && (
                        <>
                            <h1 className="text-3xl font-bold mb-1">Complete the form below!</h1>
                    ...
                    {currentStep === 5 && (
                        <>
                            ...
                            <button onClick={downloadPDF} disabled={isGenerating} className="w-full py-4 text-white rounded-lg transition font-semibold mb-4 flex items-center justify-center gap-2" style={{ backgroundColor: "#03C500" }}>
                                {isGenerating
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <button onClick={downloadHTML} className="py-3 bg-gray-100 border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-200 transition font-semibold flex items-center justify-center gap-2">
