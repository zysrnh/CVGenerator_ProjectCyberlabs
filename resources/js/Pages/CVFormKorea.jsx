...
const CVFormKorea = () => {
    ...
    const [formData, setFormData] = useState({
        full_name: '', korean_name: '', date_of_birth: '', gender: 'male',
        address: '', id_number: '', nationality: 'Indonesia', religion: 'Islam',
        height: '', weight: '', vision: '10/10', dominant_hand: 'right',
        tattoo: false, surgery: false, marital_status: 'single',
        ...
        education_level: 'highschool', school_name: '', school_address: '',
        has_seafaring_exp: false,
        ship_experiences: [{ ship_name: '', period: '', ship_nationality: '', type: '' }],
        pushups: '', situps: '', right_balance: '합격', forward_bend: '합격',
        backward_bend: '합격', hanging_seconds: '', right_grip: '', left_grip: '',
        horse_stance_seconds: '',
    });
...
<div class="page page2">
    ...
    const downloadPDF = async () => {
        if (!formData.full_name.trim()) {
            showNotification('error', 'Validasi Gagal', 'Nama lengkap wajib diisi!');
            return;
        }
        setIsGenerating(true);

        fetch('/cv-submissions-korea', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({ ...formData, photo: photoData, ...physPhotos }),
        })
            .then(r => r.json().catch(() => ({})))
            .then(result => {
                if (result.success) showNotification('success', 'Data Tersimpan!', `${formData.full_name} — berhasil disimpan`);
                else showNotification('error', 'Gagal Simpan', result.error || 'Server error');
            })
            .catch(err => showNotification('error', 'Network Error', err.message));
        ...
        try {
            ...
            showNotification('error', 'PDF Gagal', err.message);
    ...
    const prevStep = () => {
        ...
        else router.visit('/import/korea');
    };
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-12 animate-scale-in">
                    ...
                    {currentStep === 5 && (<>
                        ...
                        <p className="text-gray-500 mb-6">Step 5 — 신체 검사 결과</p>
                        ...
                        <div className="space-y-3">
                            <button onClick={downloadPDF} disabled={isGenerating} className="w-full py-4 btn-gold text-white rounded-lg font-semibold flex items-center justify-center gap-2">
                                {isGenerating
                            ...
                            <button onClick={previewCV} className="w-full py-3 bg-gray-100 border-2 border-gray-200 text-gray-700 rounded-lg hover:bg-gray-200 transition font-semibold flex items-center justify-center gap-2">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                Preview CV (buka tab baru)
                            </button>
                        </div>
                    </>)}
